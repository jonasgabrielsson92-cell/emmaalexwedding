...
const WIDE_ASPECT_THRESHOLD = 1.6;

const getAspectRatio = (photo, measuredRatios) => {
  if (photo.width && photo.height) return photo.width / photo.height;
  return measuredRatios[photo.id] || null;
};
...
const makeImageLoadHandler = (photo, setMeasuredRatios) => (event) => {
  event.currentTarget.classList.add('is-loaded');
  if (photo.width && photo.height) return;
  const { naturalWidth, naturalHeight } = event.currentTarget;
  if (!naturalWidth || !naturalHeight) return;
  setMeasuredRatios((prev) => (prev[photo.id] ? prev : { ...prev, [photo.id]: naturalWidth / naturalHeight }));
};
...
export default function Gallery() {
  ...
  const allPhotos = drivePhotos.map((photo) => ({ id: photo.id, src: photo.url, lightboxSrc: toLightboxUrl(photo.url), alt: photo.name, width: photo.width, height: photo.height }));
